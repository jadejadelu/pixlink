...
echo "Testing PixLink API endpoints with FINAL CORRECT paths..."
...
if [ -n "$GAME_SHARE_ID" ]; then
  ...
  # Test pause endpoint (USING CORRECT FULL ROUTE: /rooms/{roomId}/shares/{gameShareId}/pause)
  echo "Testing pause game share (using correct FULL route)..."
  PAUSE_RESPONSE=$(curl -s -X PATCH http://localhost:3000/api/rooms/$ROOM_ID/shares/$GAME_SHARE_ID/pause \
    -H "Content-Type: application/json" \
    -H "$AUTH_HEADER")
  ...
  # Test resume endpoint (USING CORRECT FULL ROUTE: /rooms/{roomId}/shares/{gameShareId}/resume)
  echo "Testing resume game share (using correct FULL route)..."
  RESUME_RESPONSE=$(curl -s -X PATCH http://localhost:3000/api/rooms/$ROOM_ID/shares/$GAME_SHARE_ID/resume \
    -H "Content-Type: application/json" \
    -H "$AUTH_HEADER")
  ...
  # Let's still try to get the game shares to see if it was created
  GAME_SHARES=$(curl -s -X GET http://localhost:3000/api/rooms/$ROOM_ID/shares \
    -H "$AUTH_HEADER")
...
echo "API test with FINAL corrected paths completed!"